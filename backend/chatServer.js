...
exports.start = (server) => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("disconnect", () => {
      for (const user in users) {
        if (users[user] === socket.id) {
          ...
          if (privateChat[user]) delete privateChat[user];
        }
      }
    });
    ...
    socket.on("send", (info) => {
      ...
      if (!users[info.to])
        //user is offline, no need to send live notification
        return;

      const recieverSocket = users[info.to];

      console.log(privateChat[info.to], " == ", info.from, "?");
      if (privateChat[info.to] === info.from) {
        // in private chat with user, don't send notification
